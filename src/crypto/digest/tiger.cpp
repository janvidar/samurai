...
#define SBOX1 (Samurai::Crypto::Digest::Tiger::sboxes)
#define SBOX2 (Samurai::Crypto::Digest::Tiger::sboxes + 256)
#define SBOX3 (Samurai::Crypto::Digest::Tiger::sboxes + 512)
#define SBOX4 (Samurai::Crypto::Digest::Tiger::sboxes + 768)

// #define USE_INLINED_TIGER

#ifdef USE_INLINED_TIGER
#define call_round ROUND
#define call_pass(a, b, c, mul) PASS(a, b, c, X, mul);
#define call_key_schedule() SCHEDULE(X)
#else
#define call_round round
#define call_pass pass
#define call_key_schedule() key_schedule()
#endif // USE_INLINED_TIGER

#define ROUND(A, B, C, W, M) \
	do \
	{ \
		C ^= W; \
		A -= SBOX1[(C        ) & 0xFF] ^ SBOX2[(C >> 0x10) & 0xFF] ^ \
			 SBOX3[(C >> 0x20) & 0xFF] ^ SBOX4[(C >> 0x30) & 0xFF];  \
		B += SBOX4[(C >> 0x08) & 0xFF] ^ SBOX3[(C >> 0x18) & 0xFF] ^ \
			 SBOX2[(C >> 0x28) & 0xFF] ^ SBOX1[(C >> 0x38) & 0xFF];  \
		B *= M; \
	} while(0)
...
Samurai::Crypto::Digest::Tiger::Tiger() : Hash(TIGER_HASH_SIZE, 64, false, false, 8), m_passes(3)
{
	reset();
}


Samurai::Crypto::Digest::Tiger::Tiger(size_t passes, size_t block_size) : Hash(TIGER_HASH_SIZE, block_size, false, false, 8), m_passes(passes)
{
	reset();
}
...
void Samurai::Crypto::Digest::Tiger::reset()
{
...
void Samurai::Crypto::Digest::Tiger::round(uint64_t& a, uint64_t& b, uint64_t& c, uint64_t x, uint8_t mul)
{
	c ^= x;
	a -= SBOX1[(c        ) & 0xFF] ^ SBOX2[(c >> 0x10) & 0xFF] ^
		 SBOX3[(c >> 0x20) & 0xFF] ^ SBOX4[(c >> 0x30) & 0xFF];
...
void Samurai::Crypto::Digest::Tiger::pass(uint64_t& a, uint64_t& b, uint64_t& c, uint8_t mul)
{
	round(a, b, c, X[0], mul);
	round(b, c, a, X[1], mul);
	round(c, a, b, X[2], mul);
...
void Samurai::Crypto::Digest::Tiger::key_schedule()
{
...
void Samurai::Crypto::Digest::Tiger::hash(uint8_t* data, size_t length)
{
	(void) length;
	for(int j = 0; j < 8; ++j)
		X[j] = make_uint64_t(data[8*j+7],
							 data[8*j+6],
							 data[8*j+5],
							 data[8*j+4],
							 data[8*j+3],
...
void Samurai::Crypto::Digest::Tiger::internal_finalize()
{
...
uint64_t Samurai::Crypto::Digest::Tiger::sboxes[1024] = {
